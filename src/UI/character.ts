...
export class CharacterMenu extends Menu {
	...
	refresh(opacity?: number) {
		...
		let t = $('<div>').attr('id', 'characterMenu').addClass('characterMenu').css('opacity', opacity);
		t.css('width', 800);
		...
		let w = p.w + '/' + p.W;
		$('<div>').addClass('abilitiesMenuRightTitle').text(w).appendTo(inv);
...
//			<div class="inventoryRow abilitiesRow"><div class="tooltip bottom right">精靈是帶超凡氣質的魔法民族，活在世上但又不完全屬世。他們居於飄逸之地，在古代森林之中或在閃耀妖火的銀色尖塔之中，柔和音樂乘風而轉，輕柔芳香隨風飄盪。精靈喜歡自然與魔法、美術與藝術、詩詞與歌賦、及世上一切美好之事。
	...
	draw() {
		this.refresh(0);
		this.menu.animate({opacity: 1}, 200, 'linear');
	}
